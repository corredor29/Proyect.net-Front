...
if (user) {
    ...
    document.getElementById('sbUserName').textContent = (user.firstName||'')+' '+(user.lastName||'') || user.fullName||'User';
...
function initCharts(statuses) {
    ...
    barChart = new Chart(barCtx, {
        ...
        options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{ y:{ beginAtZero:true, grid:{ color:gridColor }, ticks:{ font:{ size:11 }, color:tickColor } }, x:{ grid:{ display:false }, ticks:{ font:{ size:11 }, color:tickColor } } } }
    });
    ...
    document.getElementById('donutLegend').innerHTML = statuses.map((s,i)=>`
        <div class="legend-row"><div class="legend-left"><div class="legend-dot" style="background:${colors[i]}"></div>${s.name}</div><span class="legend-val">${s.count}</span></div>
    `).join('');
}
...
async function loadDashboard() {
    try {
        ...
        if (customers.status==='fulfilled') document.getElementById('statCustomers').textContent = (customers.value?.totalCount??customers.value?.length??'—').toLocaleString();
        if (vehicles.status==='fulfilled')  document.getElementById('statVehicles').textContent  = (vehicles.value?.totalCount??vehicles.value?.length??'—').toLocaleString();
        if (orders.status==='fulfilled') {
            const data=orders.value, items=data?.items??data??[], total=data?.totalCount??items.length;
            ...
            tbody.innerHTML = items.length===0
                ...
                : items.slice(0,8).map(o=>`<tr><td>#SO-${o.id}</td><td>${o.vehicleVin||o.vin||'—'}</td><td>${o.serviceTypeName||'—'}</td><td>${o.mechanicName||'—'}</td><td>${o.createdAt?new Date(o.createdAt).toLocaleDateString('en-US'):'—'}</td><td>${statusBadge(o.orderStatusName||o.status||'Pending')}</td></tr>`).join('');
            const statusMap={};
            items.forEach(o=>{ const s=o.orderStatusName||o.status||'Unknown'; statusMap[s]=(statusMap[s]||0)+1; });
            const statuses=Object.entries(statusMap).map(([name,count])=>({name,count}));
            initCharts(statuses.length>0?statuses:[{name:'Pending',count:0},{name:'In Progress',count:0},{name:'Completed',count:0},{name:'Cancelled',count:0}]);
        }
        if (parts.status==='fulfilled') document.getElementById('statLowStock').textContent = (parts.value?.totalCount??parts.value?.length??'—').toLocaleString();
